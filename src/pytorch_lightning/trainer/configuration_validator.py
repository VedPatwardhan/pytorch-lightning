...
def __verify_train_val_loop_configuration(trainer: "pl.Trainer", model: "pl.LightningModule") -> None:
    ...
    if causes_infinite_epochs:
        rank_zero_warn(
            "This configuration can result in infinite epochs, as a result of overriding `training_epoch_end`"
            " or `validation_epoch_end` with `val_check_interval==float`."
            " This will keep the outputs in memory indefinitely."
        )
